...
if ! printf '%s  %s' "$(grep -Eo "grep -Eq '\^[^\\]+" base/Dockerfile | sed 's/...........//')" "base/checkHealth.sh" | sha3sum -c ; then
	echo "[testStyle][ERROR]Sha3sum of checkHealth.sh in base/Dockerfile invalid"
	exit 2
fi
